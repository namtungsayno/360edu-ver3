...
export default function ClassCard({ classData, onViewDetail }) {
  ...
  const getStatusBadge = () => {
    ...
    if (status === "CANCELLED" || status === "cancelled") {
      return <Badge className="text-xs bg-red-100 text-red-800 px-1 py-0">Đã hủy</Badge>;
    }
    if (status === "AVAILABLE" || status === "available") {
      return <Badge className="text-xs bg-blue-100 text-blue-800 px-1 py-0">Đang mở</Badge>;
    }
    return <Badge className="text-xs bg-gray-100 text-gray-800 px-1 py-0">Chưa bắt đầu</Badge>;
  };
  ...
  return (
    <div className="bg-white border-2 border-gray-300 rounded-lg p-2.5 shadow-sm hover:shadow-md hover:border-blue-400 transition-all duration-200">
      ...
      {/* Subject */}
      <p className="text-xs text-gray-700 mb-1.5 truncate font-medium">{subjectName}</p>
      ...
      <div className="flex items-center gap-1.5 mb-1.5 bg-blue-50 rounded px-1.5 py-1">
        ...
        <span className="text-xs text-gray-700 truncate flex-1 font-medium">{teacherName}</span>
      </div>
      ...
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <span className="text-xs text-gray-700 font-medium">
          {studentCount}{maxStudents > 0 ? `/${maxStudents}` : ''}
        </span>
        {maxStudents > 0 && (
      ...
      <div className="flex items-center gap-1.5 mb-2">
        {isOnline ? (
          <>
            ...
            <Badge className="bg-purple-100 text-purple-800 text-xs px-1.5 py-0.5">Online</Badge>
          </>
        ) : (
          <>

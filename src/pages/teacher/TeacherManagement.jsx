...
export default function TeacherManagement() {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (!valid) {
      setError("Vui lòng nhập đầy đủ: Tên Giáo viên, Trình độ, Bộ môn, Nơi công tác.");
      return;
    }
    setError("");
    setLoading(true);
    try {
      ...
      if (form.avatarFile) {
        try {
          ...
          const uploadedUrl = await teacherUploadApi.uploadAvatar(form.avatarFile);
          // Server có thể trả về object { url: "..." } hoặc trực tiếp string URL
          payload.avatarUrl = typeof uploadedUrl === 'string' ? uploadedUrl : uploadedUrl.url;
        } catch (uploadError) {
  ...
  const addCertificate = () => {
    setCertificates(prev => [...prev, { title: "", organization: "", year: new Date().getFullYear(), description: "" }]);
  };
  ...
  const updateCertificate = (index, field, value) => {
    setCertificates(prev => prev.map((item, i) => 
      i === index ? { ...item, [field]: value } : item
    ));
  };

  const addEducation = () => {
    setEducations(prev => [...prev, { degree: "", school: "", year: new Date().getFullYear(), description: "" }]);
  };
  ...
  const addExperience = () => {
    setExperiences(prev => [...prev, { position: "", company: "", startYear: new Date().getFullYear(), endYear: null, description: "" }]);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      {loading && !form.fullName ? (
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <form id="teacher-profile-form" className="mt-6 space-y-6" onSubmit={onSubmit}>
            {/* THÔNG TIN CƠ BẢN */}
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              <h3 className="font-medium text-gray-900 mb-4">📝 Thông tin mô tả</h3>
            ...
            <div className="bg-green-50 rounded-lg p-4">
              <h3 className="font-medium text-gray-900 mb-4">🔗 Liên hệ và Hình ảnh</h3>
          ...
          <div className="mt-8 border-t pt-6">
            ...
            <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <Input
                        type="number"
                        value={cert.year || ""}
                        onChange={(e) => updateCertificate(index, "year", parseInt(e.target.value) || null)}
                        placeholder="Năm"
                        className="text-sm"
                      />
                    </div>
              ...
              {certificates.length === 0 && (
                <p className="text-gray-500 text-center text-sm py-3">Chưa có chứng chỉ</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-6 border-t pt-6">
            ...
            <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <Input
                        type="number"
                        value={edu.year || ""}
                        onChange={(e) => updateEducation(index, "year", parseInt(e.target.value) || null)}
                        placeholder="Năm"
                        className="text-sm"
                      />
                    </div>
              ...
              {educations.length === 0 && (
                <p className="text-gray-500 text-center text-sm py-3">Chưa có học vấn</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-6 border-t pt-6">
            ...
            <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Input
                        type="number"
                        value={exp.startYear || ""}
                        onChange={(e) => updateExperience(index, "startYear", parseInt(e.target.value) || null)}
                        placeholder="Từ năm"
                        className="text-sm"
                      />
                      <Input
                        type="number"
                        value={exp.endYear || ""}
                        onChange={(e) => updateExperience(index, "endYear", parseInt(e.target.value) || null)}
                        placeholder="Đến năm"
                        className="text-sm"
                      />
                    </div>
              ...
              {experiences.length === 0 && (
                <p className="text-gray-500 text-center text-sm py-3">Chưa có kinh nghiệm</p>
              )}
            </div>
          </div>
        ...
        <div className="p-6">
          <h2 className="text-xl font-semibold text-gray-900">
            ...
            <div className="mt-6">
                ...
                <div className="flex-1">
                  ...
                  {(form.facebookUrl || form.linkedinUrl) && (
                    <div className="flex gap-3 mt-3">
                      {form.facebookUrl && (
                        <a href={form.facebookUrl} target="_blank" rel="noopener noreferrer" 
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {form.specialization && (
                  <div className="rounded-lg border border-gray-200 p-4">
                    ...
                    <p className="mt-1 text-gray-900">{form.specialization}</p>
                  </div>
                )}
              ...
              {certificates.length > 0 && (
                <div className="mt-6">
                  <p className="text-lg font-semibold text-gray-900 mb-4">🏆 Chứng chỉ</p>
                  <div className="space-y-3">
                    {certificates.map((cert, index) => (
                      <div key={index} className="rounded-lg border border-gray-200 p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-semibold text-gray-900">{cert.title || "Chứng chỉ"}</h4>
                            {cert.organization && (
                              <p className="text-sm text-gray-600">{cert.organization}</p>
                            )}
                            {cert.year && (
                              <p className="text-sm text-blue-600">Năm {cert.year}</p>
                            )}
                            {cert.description && (
              ...
              {educations.length > 0 && (
                <div className="mt-6">
                  <p className="text-lg font-semibold text-gray-900 mb-4">🎓 Học vấn</p>
                  <div className="space-y-3">
                    {educations.map((edu, index) => (
                      <div key={index} className="rounded-lg border border-gray-200 p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-semibold text-gray-900">{edu.degree || "Bằng cấp"}</h4>
                            {edu.school && (
                              <p className="text-sm text-gray-600">{edu.school}</p>
                            )}
                            {edu.year && (
                              <p className="text-sm text-blue-600">Tốt nghiệp {edu.year}</p>
                            )}
                            {edu.description && (
              ...
              {experiences.length > 0 && (
                <div className="mt-6">
                  <p className="text-lg font-semibold text-gray-900 mb-4">💼 Kinh nghiệm làm việc</p>
                  <div className="space-y-3">
                    {experiences.map((exp, index) => (
                      <div key={index} className="rounded-lg border border-gray-200 p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-semibold text-gray-900">{exp.position || "Vị trí"}</h4>
                            {exp.company && (
      ...
      <div className="mt-6 bg-white border border-gray-200 rounded-lg">
        <div className="p-6">
          <div className="flex items-center justify-center gap-3">
            <Button type="submit" disabled={loading || uploadingImage || !valid} form="teacher-profile-form" className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium">
              {uploadingImage ? "📤 Đang upload ảnh..." : loading ? "⏳ Đang lưu..." : "💾 Lưu hồ sơ"}
            </Button>
            {saved && (
              <span className="text-lg text-green-700 font-medium">✅ Đã lưu thành công!</span>
            )}
          </div>
        </div>
      </div>
